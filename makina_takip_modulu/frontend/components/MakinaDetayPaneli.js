...
export default function MakinaDetayPaneli({ makina, onClose, onEdit, onDelete, onFinishWork, onGoToLocation }) {
    ...
    const loadMakinaIsleri = async () => {
        ...
        try {
            ...
            const apiUrl = `http://199.241.137.77:60/api/sulama/makina-isler/?makina=${makina.id}`;
    ...
    const handleFinishWork = async (makina) => {
        try {
            ...
            // İşi tamamla
            const response = await fetch(`http://199.241.137.77:60/api/sulama/makina-isler/${aktifIs.id}/is_tamamla/`, {
                method: 'POST',
                headers: {
                    'Authorization': `Token ${token}`,
                    'Content-Type': 'application/json',
